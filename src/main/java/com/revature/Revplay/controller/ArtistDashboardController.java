...
this.userRepository = userRepository;
    ...
    public String artistDashboard(
        ...
        List<com.revature.Revplay.entity.PodcastEpisode> myEpisodes = podcastEpisodeRepository.findByPodcast_Artist_ArtistIdOrderByCreatedAtDesc(artist.getArtistId());

        // Build albumId -> coverImage map for reliable lookup in templates
        java.util.Map<Long, String> albumCoverMap = new java.util.HashMap<>();
        for (Album a : myAlbums) {
            if (a.getCoverImage() != null) {
                albumCoverMap.put(a.getAlbumId(), a.getCoverImage());
            }
        }
        ...
        model.addAttribute("myEpisodes", myEpisodes);
        model.addAttribute("albumCoverMap", albumCoverMap);
