...
public interface SongRepository extends JpaRepository<Song, Long> {
	List<Song> findTop6ByOrderByPlayCountDesc();
	...
	@Query("SELECT s FROM Song s LEFT JOIN FETCH s.album WHERE s.artist.artistId = :artistId ORDER BY s.playCount DESC")
	java.util.List<Song> findTop5ByArtist_ArtistIdOrderByPlayCountDesc(@Param("artistId") Long artistId);
	
	@Query("SELECT s FROM Song s LEFT JOIN FETCH s.album WHERE s.artist.artistId = :artistId")
	List<Song> findByArtist_ArtistId(@Param("artistId") Long artistId);
