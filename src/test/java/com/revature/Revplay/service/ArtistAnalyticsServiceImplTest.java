...
public class ArtistAnalyticsServiceImplTest {
    ...
    @Mock private PodcastRepository podcastRepository;               // ← added
    @Mock private PodcastFavoriteRepository podcastFavoriteRepository; // ← added
    ...
    public void setUp() {
        ...
        // Default stubs for podcast repos so all tests pass without explicit setup
        when(podcastRepository.countByArtist_ArtistId(anyLong())).thenReturn(0L);
        when(podcastRepository.findByArtist_ArtistId(anyLong())).thenReturn(Collections.emptyList());
        when(podcastFavoriteRepository.countByArtistId(anyLong())).thenReturn(0L);
    ...
    @Test
    public void testGetAnalytics_WithPodcasts_ReturnsPodcastCount() {
        logger.info("Running testGetAnalytics_WithPodcasts_ReturnsPodcastCount");

        Podcast p1 = new Podcast();
        p1.setPodcastId(10L);
        p1.setTitle("Tech Pod");

        when(artistRepository.findById(1L)).thenReturn(Optional.of(testArtist));
        when(songRepository.countByArtist_ArtistId(1L)).thenReturn(3L);
        when(songRepository.getTotalLikes(1L)).thenReturn(50L);
        when(podcastRepository.countByArtist_ArtistId(1L)).thenReturn(1L);
        when(podcastRepository.findByArtist_ArtistId(1L)).thenReturn(Arrays.asList(p1));
        when(podcastFavoriteRepository.countByArtistId(1L)).thenReturn(5L);
        when(podcastFavoriteRepository.countByPodcastId(10L)).thenReturn(5L);
        when(listeningRepository.getTotalPlays(1L)).thenReturn(200L);
        when(listeningRepository.getPlaysAfter(eq(1L), any())).thenReturn(20L);
        when(songRepository.findTop5ByArtist_ArtistIdOrderByPlayCountDesc(1L))
                .thenReturn(Collections.emptyList());

        ArtistAnalyticsDTO result = analyticsService.getAnalytics(1L);

        assertEquals(Long.valueOf(1L), result.getTotalPodcasts());
        assertEquals(1, result.getPodcasts().size());
        assertEquals("Tech Pod", result.getPodcasts().get(0).getTitle());
    }
}